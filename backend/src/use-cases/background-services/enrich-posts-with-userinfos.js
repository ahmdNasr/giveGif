...
async function enrichPostsWithUserInfos({ posts }) {
  const allUserIds = extractUserIdsFromPostsAndReplies(posts);
  ...
  const enrichedPosts = mapPostedByIdsToUserObjectsRecursive(
    posts,
